...
csp = mne.decoding.CSP(n_components=num_features, cov_est='epoch', transform_into='csp_space')
...
def next_batch(self, sess, flag):
  ...
  data = np.zeros([batch_size, step_size, num_features])
  ...
  for ind, filename in enumerate(batch_files):
    ...
    rdata = (mycsp.transform(X))
      # Data Pre-Processing
      # Normalization
    data[ind, :, :] = np.multiply(rdata - (rdata.mean(0).reshape([1,num_features])), 1/(10e-12 + rdata.std(0).reshape([1,5])))
    f_temp = open(filepath_lab,'r')
    lab_temp = f_temp.read()
    f_temp.close()
    if lab_temp=='Like':
